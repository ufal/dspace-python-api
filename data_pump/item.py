...
def import_item(metadata_class,
    ...
    def import_item(metadata_class,
        ...
        statistics_dict['versions_imported'] = (0, len(processed_items_id))
        statistics_dict['versions_not_imported_withdrawn'] = (0, len(withdrawn_item_handles))
        statistics_dict['versions_not_imported_error'] = (0, len(not_imported_item_handles))
    ...
    def check_sum(c7_dspace, c5_dspace, item_id_dict, withdrawn_item_handles, not_imported_item_handles,
        ...
        missing_item_ids = []
        # Check if non-imported item is in the workflow state - unfinished version of the item or if it is not withdrawn
        # Check if it is in workflow
        for item_uuid in not_imported_items:
            item_id = item_handle_item_metadata_dict[item_uuid]
        ...
        if len(worklfowitem_not_imported) == len(not_imported_items):
            logging.info('All non-imported items are workflow items.')
            logging.info('All versions from all items are successfully migrated.')
        else:
            logging.error('Version migration was not successful for these items: ')
            for non_imported_uuid in not_imported_items:
                logging.error(f'Cannot migrate version for this item: {non_imported_uuid}')
    ...
    def get_item_version_sequence(item_id,
        ...
        previous_versions = get_item_versions(item_id, items_dict, metadata_class, item_handle_item_metadata_dict,
                                              False,
                                              withdrawn_item_handles, not_imported_item_handles)
        ...
        while current_version is not None:
            if current_version not in item_handle_item_metadata_dict:
                ...
                if item['withdrawn']:
                    ...
                    logging.info(
                        f'The item with handle: {current_version} is withdrawn and will not be imported because '
                        f'it is stored in another repository.')
