...
def import_epersongroup(metadata_class, group_id_dict, statistics_dict):
    """
    ...
    if existing_data_dict:
        for i in existing_data_dict:
            if i['name'] == 'Anonymous':
                group_id_dict[0] = [i['id']]
            elif i['name'] == 'Administrator':
                group_id_dict[1] = [i['id']]
            else:
    ...
    if not group_json_a:
        ...
        return
    for i in group_json_a:
        id = i['eperson_group_id']
        # group Administrator and Anonymous already exist
        # group is created with dspace object too
        if id != 0 and id != 1 and id not in group_id_dict:
            # get group metadata
            metadatavalue_group_dict = metadata_class.get_metadata_value(6,
                                                                         i['eperson_group_id'])
...
def import_group2group(group_id_dict, statistics_dict):
    """
    Import data into database.
    ...
    if not group2group_json_a:
        ...
        return
    for i in group2group_json_a:
        parents_a = group_id_dict[i['parent_id']]
        childs_a = group_id_dict[i['child_id']]
