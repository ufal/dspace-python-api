...
def import_bundle(metadata_class, item_id_dict, bundle_id_dict, primary_bitstream_dict,
                  statistics_dict):
    """
    ...
    item2bundle_dict = dict()
    if not item2bundle_json_a:
        logging.info("Item2bundle JSON is empty.")
        return
    for i in item2bundle_json_a:
        if i['item_id'] in item2bundle_dict:
            item2bundle_dict[i['item_id']].append(i['bundle_id'])
        else:
            item2bundle_dict[i['item_id']] = [i['bundle_id']]
    ...
    if not bundle_json_a:
        ...
        return
    for i in bundle_json_a:
        if i['primary_bitstream_id']:
            primary_bitstream_dict[i['primary_bitstream_id']] = i['bundle_id']
