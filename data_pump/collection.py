...
def import_collection(metadata_class, handle_class, group_id_dict, community_id_dict,
                      collection_id_dict, collection2logo_dict, statistics_dict):
    """
    ...
    if not comm2coll_json_a:
        ...
        return
    for i in comm2coll_json_a:
        coll2comm_dict[i['collection_id']] = i['community_id']

    # because the role DEFAULT_READ is without old group id in collection
    coll2group_dict = dict()
    metadata_json_a = read_json(metadata_json_name)

    if metadata_json_a:
        for i in metadata_json_a:
            if i['resource_type_id'] == 6 and 'COLLECTION_' in i['text_value'] \
                    and '_DEFAULT_READ' in i['text_value']:
                text = i['text_value']
    ...
    if not collection_json_a:
        ...
        return
    for i in collection_json_a:
        collection_json_p = {}
        metadata_col_dict = metadata_class.get_metadata_value(3, i['collection_id'])
        if metadata_col_dict:
            collection_json_p['metadata'] = metadata_col_dict
        handle_col = handle_class.get_handle(3, i['collection_id'])
        if handle_col:
            collection_json_p['handle'] = handle_col['handle']
        params = {'parent': community_id_dict[coll2comm_dict[i['collection_id']]]}
        try:
            response = do_api_post(collection_url, params, collection_json_p)
            coll_id = convert_response_to_json(response)['id']
            collection_id_dict[i['collection_id']] = coll_id
            imported_coll += 1
        except Exception:
            logging.error(
                'POST request ' + response.url + ' for id: ' + str(i['collection_id']) +
                'failed. Status: ' +
                str(response.status_code))

        # add to collection2logo, if collection has logo
        if i['logo_bitstream_id'] is not None:
            collection2logo_dict[i['collection_id']] = i["logo_bitstream_id"]
        ...
        if i['workflow_step_2']:
            try:
                ...
                group_id_dict[i['workflow_step_2']] = [
        ...
        if i['submitter']:
            try:
                response = do_api_post(collection_url + coll_id + '/submittersGroup',
                                       None, {})
                group_id_dict[i['submitter']] = \
                    [convert_response_to_json(response)['id']]
                imported_group += 1
            except Exception:
                logging.error('POST request ' + response.url +
                              ' failed. Status: ' + str(response.status_code))
        if i['collection_id'] in coll2group_dict:
            try:
                response = do_api_post(collection_url + coll_id +
                                       '/bitstreamReadGroup', None, {})
                group_id_dict[coll2group_dict[i['collection_id']]] = [
                    convert_response_to_json(response)['id']]
                imported_group += 1
            except Exception:
                logging.error('POST request ' + response.url +
            ...
            try:
                response = do_api_post(collection_url + coll_id +
                                       '/itemReadGroup', None, {})
                group_id_dict[coll2group_dict[i['collection_id']]].append(
                    convert_response_to_json(response)['id'])
                imported_group += 1
            except Exception:
                logging.error('POST request ' + response.url +
